...
function getTime() {
    ...
    time.getHours() < 10 ? hour.textContent = `0${time.getHours()}` : hour.textContent = time.getHours()
    time.getMinutes() < 10 ? min.textContent = `0${time.getMinutes()}` : min.textContent = time.getMinutes()
    time.getSeconds() < 10 ? sec.textContent = '0' + time.getSeconds() : sec.textContent = time.getSeconds()
}
setInterval(getTime, 1000);


const controls = document.querySelectorAll('.controls>div>input')
function controlsFun() {
    const suffix = this.dataset.bg || ''
    console.log(suffix)
    document.documentElement.style.setProperty(`--${this.name}`, this.value + suffix)
    console.log(this.name + suffix)

}
controls.forEach(control => control.addEventListener('change', controlsFun))
controls.forEach(control => control.addEventListener('mousemove', controlsFun))